...
export const dbHelpers = {
  ...
  getSessionsByDateRange: async (startDate: Date, endDate: Date) => {
    return await db.sessions.where('date').between(startDate, endDate, true, true).toArray()
  },
  ...
  getStats: async (): Promise<WorkoutStats> => {
    ...
    const weightCounts = weights.reduce(
      (acc, weight) => {
    ...
    const mostUsedWeight =
      Object.keys(weightCounts).length > 0
        ? Object.keys(weightCounts).reduce((a, b) =>
            (weightCounts[Number(a)] || 0) > (weightCounts[Number(b)] || 0) ? a : b
          )
        : '0'

    // Workouts by day of week
    const workoutsByDay = sessions.reduce(
      (acc, session) => {
    ...
    const weightOverTime = recentSessions
      .filter((session) => session.date)
      .map((session) => ({
        date: session.date.toISOString().split('T')[0],
        weight: session.exercises.reduce(
          (sum, exercise) =>
